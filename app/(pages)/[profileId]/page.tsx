      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-20 flex flex-col lg:flex-row gap-10 lg:gap-20">
        ...
        <div className="w-full lg:w-[70%]">
             <div className="mb-6 pl-2 border-l-4 border-accent-green">
                <h2 className="text-xl font-bold text-content-heading">Meus Links & Projetos</h2>
                <p className="text-sm text-content-body">Explore meu conteúdo mais recente</p>
             </div>

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {isOwner && (
                    <div className="w-full flex justify-center items-center md:items-stretch">
                        <NewProject profileId={profileId} />
                    </div>
                )}

                {projectsWithImages.map((project) => (
                    <div key={project.id} className="w-full flex justify-center items-center md:items-stretch animate-fade-in-up">
                        <ProjectCard
                            project={project}
                            isOwner={isOwner}
            ...
            {!isOwner && projects.length === 0 && (
                <div className="text-center py-20 bg-background-secondary rounded-2xl border-dashed border-2 border-border-secondary opacity-70">
                    <p className="text-content-body">Nenhum projeto publicado ainda.</p>
                </div>
            )}
        </div>
